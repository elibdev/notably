export interface TableInfo { name: string; createdAt: string }
...
export class ApiClient {
  ...
  async getRow(table: string, id: string): Promise<RowData> {
    const res = await fetch(
      `/api/tables/${encodeURIComponent(table)}/rows/${encodeURIComponent(
        id
      )}`,
      { headers: this.headers() }
    );
